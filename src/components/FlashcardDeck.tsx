...
const colorMap: Record<string, { bg: string; hover: string; text: string; border: string; subtext: string }> = {
  rose:    { bg: "bg-rose-500/12",    hover: "hover:bg-rose-500",    text: "text-rose-400",    border: "border-rose-500/25",    subtext: "text-rose-300/70" },
  amber:   { bg: "bg-amber-500/12",   hover: "hover:bg-amber-500",   text: "text-amber-400",   border: "border-amber-500/25",   subtext: "text-amber-300/70" },
  emerald: { bg: "bg-emerald-500/12", hover: "hover:bg-emerald-500", text: "text-emerald-400", border: "border-emerald-500/25", subtext: "text-emerald-300/70" },
  blue:    { bg: "bg-[#0071e3]/12",   hover: "hover:bg-[#0071e3]",   text: "text-[#0071e3]",   border: "border-[#0071e3]/25",   subtext: "text-blue-300/70" },
};
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[85vh] text-center px-4 py-6">
        <motion.div
          ...
          className="bg-zinc-900/80 backdrop-blur-2xl border border-zinc-800/80 p-6 sm:p-10 rounded-3xl shadow-2xl max-w-sm w-full"
        >
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            transition={{ type: "spring", damping: 12, delay: 0.2 }}
            className="w-16 h-16 sm:w-20 sm:h-20 bg-emerald-500/10 text-emerald-400 rounded-2xl sm:rounded-3xl flex items-center justify-center mx-auto mb-5 border border-emerald-500/20"
          >
          ...
          <p className="text-xs sm:text-sm text-zinc-400 mb-6 leading-relaxed">
            {cards.length === 0 ? (
              ...
              <span className="font-bold text-emerald-400">assimilados</span>
          ...
          {completedCount > 0 && (
            <div className="grid grid-cols-2 gap-2 mb-5">
              <div className="bg-zinc-800/60 p-3 rounded-xl border border-zinc-700/40">
          ...
          <Link
            href="/"
            className="w-full py-3.5 px-4 bg-[#0071e3] hover:bg-[#005bb5] text-white font-extrabold rounded-xl transition text-center shadow-lg shadow-[#0071e3]/20 block text-sm min-h-[48px] flex items-center justify-center gap-2 touch-manipulation"
          >
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-4 sm:py-8 flex flex-col items-center relative min-h-screen">
        ...
        {levelUpModal && (
          ...
          >
            <motion.div
              ...
              className="bg-zinc-900 border-2 border-amber-500/30 p-7 sm:p-8 rounded-3xl shadow-2xl max-w-xs w-full text-center relative overflow-hidden"
            >
              ...
              <span className="relative z-10 text-[10px] sm:text-xs uppercase font-extrabold tracking-widest text-amber-400">{`Novo n\u00edvel alcan\u00e7ado!`}</span>
              <h2 className="relative z-10 text-2xl sm:text-3xl font-black text-white mt-1 mb-1">{`N\u00edvel`} {levelUpModal.level}</h2>
              <p className="relative z-10 text-base sm:text-lg font-bold text-cyan-400 mb-6">{levelUpModal.title}</p>

              <button
                onClick={() => setLevelUpModal(null)}
                className="relative z-10 w-full py-3.5 bg-gradient-to-r from-amber-400 to-yellow-500 text-zinc-950 font-black rounded-xl shadow-lg active:scale-95 transition min-h-[48px] touch-manipulation"
              >
                {"Continuar estudando \uD83D\uDD25"}
              </button>
            </motion.div>
      ...
      <div className="w-full flex items-center justify-between gap-2 mb-3 sm:mb-5">
        <Link
          href="/"
          className="flex items-center gap-1.5 text-xs font-semibold text-zinc-500 hover:text-white transition bg-zinc-900/80 px-3 py-2 rounded-xl border border-zinc-800/80 min-h-[40px] touch-manipulation"
        >
          <ArrowLeft size={14} />
        ...
        <div className="flex items-center gap-1.5">
          {currentCard.deck && (
            <span className="hidden sm:inline-flex items-center gap-1 px-2.5 py-1.5 bg-purple-500/10 text-purple-400 rounded-lg text-[10px] font-bold border border-purple-500/15 truncate max-w-[100px]">
              <Layers size={11} /> {currentCard.deck.title}
            </span>
          )}
          {streak > 0 && (
            <span className="flex items-center gap-1 px-2 py-1.5 bg-amber-500/10 text-amber-400 rounded-lg text-[10px] font-bold border border-amber-500/15">
              <Flame size={11} /> {streak}
            </span>
          )}
          <span className="flex items-center gap-1 px-2 py-1.5 bg-zinc-800/80 text-zinc-400 rounded-lg text-[10px] font-bold border border-zinc-700/40">
      ...
      <div className="w-full mb-5 sm:mb-6">
        <div className="w-full h-1 sm:h-1.5 bg-zinc-800/80 rounded-full overflow-hidden">
          <motion.div
      ...
      >
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="w-full min-h-[280px] sm:min-h-[340px] rounded-2xl sm:rounded-3xl p-5 sm:p-8 bg-zinc-900/70 backdrop-blur-xl border border-zinc-800/80 shadow-[0_12px_40px_rgba(0,0,0,0.5)] flex flex-col items-center justify-center text-center"
          >
            {/* Card Image */}
            {currentCard.imageUrl && (
              <div className="mb-4 max-h-32 sm:max-h-40 rounded-xl overflow-hidden border border-zinc-800/60">
                <img
            ...
            {!isFlipped ? (
              ...
              <div className="flex flex-col items-center justify-center w-full py-2">
                ...
                <div className="text-lg sm:text-2xl font-extrabold text-white leading-snug break-words max-w-full prose-card">
                  <ReactMarkdown remarkPlugins={[remarkGfm]}>{currentCard.front}</ReactMarkdown>
                </div>
                <p className="text-[10px] text-zinc-600 mt-6 flex items-center gap-1.5">
              ...
              /* ─── BACK ─── */
              <div className="flex flex-col items-center justify-center w-full py-2">
                <span className="text-[9px] sm:text-[10px] uppercase tracking-widest font-extrabold text-emerald-400 bg-emerald-500/8 px-2.5 py-1 rounded-full mb-3 border border-emerald-500/15">
                  Resposta
                </span>
                <div className="text-base sm:text-xl font-bold text-white leading-relaxed mb-3 break-words max-w-full prose-card">
                  <ReactMarkdown remarkPlugins={[remarkGfm]}>{currentCard.back}</ReactMarkdown>
                </div>
                {currentCard.extra && (
                  <div className="text-[11px] sm:text-xs text-zinc-400 italic bg-zinc-800/50 p-3 rounded-xl border border-zinc-700/30 max-w-sm w-full prose-card">
            ...
            {ANSWER_BUTTONS.map((btn) => {
              ...
              return (
                <button
                  ...
                  disabled={isSyncing}
                  className={`group flex flex-col items-center py-2.5 sm:py-3.5 px-1 rounded-xl sm:rounded-2xl ${c.bg} ${c.hover} ${c.text} hover:text-white border ${c.border} transition-all duration-150 active:scale-95 touch-manipulation min-h-[56px] justify-center disabled:opacity-50`}
                >
                  ...
                  <span className="text-[8px] text-zinc-600 font-mono hidden sm:block">[{btn.key}]</span>
                </button>
              );
            })}
          </motion.div>
